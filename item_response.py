...
def _A(data, theta, beta, i, j): return (2 * data[i][j] - 1) * (theta[i] - beta[j])

# w.r.t theta
def _dA_theta(data, i, j): return (2 * data[i][j] - 1)

# w.r.t beta
def _dA_beta(data, i, j): return -(2 * data[i][j] - 1)
...
def main():
    ...
    plt.plot(x_axis, val_acc_lst, label="IRT model", color="blue")
    plt.xlabel("iterations")
    plt.ylabel("score")
    plt.legend()
    plt.show()

    plt.plot(x_axis, nllk_lst, label="IRT model", color="green")
    plt.xlabel("iterations")
